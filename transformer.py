...
train_data["ori_len"] = train_ori["ori_len"]
val_data["ori_len"] = val_ori["ori_len"]
...
class TransformerDataset(Dataset):
    ...
    def __init__(self,
                 embed_list, label_list, len_list):
    ...
    def __getitem__(self, index):
        ...
        embed = torch.Tensor(embed).view(200, 50)
        length = self.len_list[index]
        length = torch.Tensor([length for i in range(200)]).view(200, 1)
        length = torch.cat([length for i in range(5)], dim=1)
        # embed = torch.cat([length, embed], dim=1)
...
class Transformer_for_classify(torch.nn.Module):
    def __init__(self, d_model=50, nhead=5, num_encoder_layers=4, dim_feedforward=100, dropout=0):
        super(Transformer_for_classify, self).__init__()
        self.encoder_layer = torch.nn.TransformerEncoderLayer(d_model=d_model, nhead=nhead,
                                                              dim_feedforward=dim_feedforward, dropout=dropout,
                                                              batch_first=True)
        self.transformer = torch.nn.TransformerEncoder(encoder_layer=self.encoder_layer, num_layers=num_encoder_layers)
        self.linear = torch.nn.Linear(d_model, 2)
        self.softmax = torch.nn.Softmax(dim=1)

    def forward(self, src):
        output = self.transformer(src).permute(1, 0, 2)[0]
...
def test(model, val_loader, criterion, device, epoch):
    ...
    global best_acc
    if avg_acc > best_acc:
        torch.save(model.state_dict(), "../model/transformer_cat.pth")
        best_acc = avg_acc
        print("======Saving model======")
...
def main():
    ...
    print('Using', device)
    model = Transformer_for_classify().to(device)
    train_dataset = TransformerDataset(train_data['token'], train_data['label'], train_data['ori_len'])
    val_dataset = TransformerDataset(val_data['token'], val_data['label'], val_data['ori_len'])
    ...
    if not os.path.exists("../pic"):
        os.mkdir("../pic")
    plt.savefig("../pic/transformer_cat.png")
...
def main_test():
    ...
    train_dataset = TransformerDataset(train_data['token'], train_data['label'], train_data['ori_len'])
    val_dataset = TransformerDataset(val_data['token'], val_data['label'], train_data['ori_len'])
    train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=16, shuffle=True)
    val_loader = torch.utils.data.DataLoader(val_dataset, batch_size=16, shuffle=False)
    criterion = torch.nn.CrossEntropyLoss()
